...
After 3D primitives have been placed and aligned with the generated 2D silhouette contours, these 3D primitives should be deformed so that their 2D silhouette contours can match the generated 2D silhouette contours exactly. Here we use the example shown in Figure \ref{fig:FDMnode}to demonstrate the algorithm of our proposed primitive deformer and how it deforms a 3D primitive to match the 2D silhouette contours. 
\begin{figure}[h]
\centering
...
\centering
 ...
 \label{fig:blend}
\end{figure}  
With the above surface blending method, the transition surfaces between the deformed primitives shown in Figure  \ref{fig:system_overview}\subref{fig:primitives_after_deformation} are created. A smooth rough 3D base mesh is obtained in Figure \ref{fig:system_overview}\subref{fig:blending_body_parts}.
...
The user interface of our developed shape generator uses different windows. They are used to respectively treat local model creation from sketches in different view planes.\\
For local model creation from sketches in three view planes, four windows are used to help provide depth information and control the shape of 3D local details to be created. As shown in Figure \ref{fig:detail_UI}, the two top windows and bottom left window allow users to sketch 2D silhouette contours from two or all of three orthotropic views: front view, side view, and top view in the local coordinates of a local shape, or project a 3D model to the three orthotropic views to modify them. The bottom right window is used to display the created 3D models.\\
\begin{figure}[h]
\centering
...
\caption{Interface for sketching three orthotropic silhouette contours and viewing generated 3D models}
 \label{fig:detail_UI}
\end{figure}
Taking a chest model as an example, we first sketch a 2D silhouette contour in the front view window as indicted in Figure \ref{fig:detail_UI}(a). The sketched silhouette contour is displayed in the side view window. Then we sketch the second 2D silhouette contour in the side view window shown in Figure \ref{fig:detail_UI}(b) which intersects with the 2D silhouette contour in the front view window. Both the 2D silhouette contours sketched in the front and side view windows are displayed in the top view window. In the top view window shown in Figure \ref{fig:detail_UI}(c), we sketch the third 2D silhouette contour which intersects with the two sketched silhouette contours respectively obtained from the front view and the side view. Once a 3D model is created from the two or three sketched silhouette contours, it is displayed in the bottom right window Figure \ref{fig:detail_UI}(d). \\
...
The base mesh generated from deformed primitives represent a rough 3D models, 3D details should be added to the rough base mesh to generate a detailed 3D model. In this section, we will develop an ODE-driven shape generator to create various local shapes from user?s drawn sketches in different view planes. These sketches can be divided into: two open sketches, one open and one closed sketches, and two open and one closed sketches.  In the following three subsections, we will investigate the three algorithms to create 3D shapes from these three different types of sketches. 
...
\end{subfigure}
\caption{Generation of 3D shapes from two open silhouette contours in two different view planes}
\label{fig:male_leg}
\end{figure}\\
...
Using the same treatment, we can obtain the generator at the other positions. From these obtained generators, a local detail 3D model is created and depicted in Figure \ref{fig:shoulder_armor}.\\
\begin{figure}[!htb]
\centering
\begin{subfigure}{0.24\linewidth}
\centering
...
The algorithms given in Subsections \ref{algorithm_2_opens} and \ref{algorithm_1_open_1_closed} enable users to create local 3D shapes from the sketches in two orthotropic view planes. By adding one or more silhouette contours in the third orthotropic view plane, users can further control the shape of 3D models.In this subsection, we discuss construct a 3D detail model from two open and one closed silhouette contours in three orthotropic view planes.\\
As shown in Figure \ref{fig:2open1close}, the task of creating a local 3D shape passing through two open and one closed silhouette contours can be transformed into constructing 4 sweeping surfaces encircled by the curves $\mathbf{c_1c_6c_5}$, $\mathbf{c_2c_7c_6}$, $\mathbf{c_3c_8c_7}$, and $\mathbf{c_4c_5c_8}$ respectively.\\
\begin{figure}[h]
	\begin{center}
...
Using the same treatment, we obtain all the curves of the generator at the positions $j=1,2,3,\ldots, J-2, J-1, J$. They are used to create the sweeping surface.% shown in Figure \ref{fig:2open1close}.\\
\begin{figure}[!h]
\centering
  % Fixed length
 ...
 \label{fig:mouth}
\end{figure}  \\
With the above method, we draw a closed lip contour and two open curves shown in Figure \ref{fig:mouth}\subref{fig:mouth_contour_front} and \ref{fig:mouth}\subref{fig:mouth_contour_side}. The two open curves divide the mouth into four regions. One surface is created for each of the four regions. Figure \ref{fig:mouth}\subref{fig:mouth_persp} depicts the mouth model consisting of the four surfaces.\\
\begin{figure}[!h]
\centering
...
\end{figure} \\
The above method can be also extended to deal with the situations where the two open curves do not intersect. For such situations, the intersecting point of the two open curves becomes the upper closed curve as indicated in Figure \ref{fig:4opens2closesdescribe}, and a sweeping surface is encircled by four curves. \\
With this extension, we created a 3D vest model and an eye socket model depicted it in Figure \ref{fig:4opens2closed}\subref{fig:vest} and Figure \ref{fig:4opens2closed}\subref{fig:eye}, respectively. \\
\begin{figure}[!h]
  % Fixed length
...
\end{figure}  
The extended algorithm is not only applicable to top and bottom closed curves, but also top and bottom open curves. Figure \ref{fig:arm_armor} gives such an example where the three curves in red are open as shown in Figure  \ref{fig:arm_armor}\subref{fig:arm_armor_contour}. The generated 3d model is depicted in Figure \ref{fig:arm_armor}\subref{fig:arm_armor_mesh}.
\begin{figure}[!h]
\centering
  % Fixed length
...
\section{Experiment and comparisons}
In this section, we present an experiment to validate our present ODE-driven deformations can create more realistic shapes and use the creation of a 3D facial model to demonstrate our proposed approach can create detailed 3D models more efficiently.\\
The experiment is to deform a tor tyre and compare its ground-truth deformations with those determined by our developed ODE-driven deformations.\\
\begin{figure}[!h]
\centering
  % Fixed length
  ...
  \caption{Deformation comparison:Deformation comparison: (a). unreformed toy tyre with an inner diameter 3 cm, (b). deformed toy tyre by moving the horizontal diameter to 3.6 cm, (c) comparison of the inner curves: black-unreformed, red-deformed, pink-ODE-driven deformation, and green-ellipse.}
 \label{fig:tyre}
\end{figure} 
As shown in Figure \ref{fig:tyre}, the inner curve of the toy tyre is a circle, i. e., the black one in Figure \ref{fig:tyre}\subref{fig:tyre_contour}. The diameter of the circle has a diameter of 3 cm. After moving the horizontal diameter to 3.6 cm, the toy tyre in Figure \ref{fig:tyre}\subref{fig:tyre0} is deformed into the shape shown in Figure \ref{fig:tyre}\subref{fig:tyre1}, and the inner curve is changed from the black circle into the red curve shown in Figure \ref{fig:tyre}\subref{fig:tyre_contour}. With our developed ODE-driven deformations and setting the displacement differences $d_1=d_{N+1}=0.3 cm$, the black circle is deformed into the pink curve. If a standard ellipse is used to describe the deformed circle, we obtain the green curve shown in Figure \ref{fig:tyre}\subref{fig:tyre_contour}.\\ 
By comparing the red, pink, and green curves in Figure \ref{fig:tyre}\subref{fig:tyre_contour}, the pink curve is much closer to the red curve than the green curve. It indicates that ODE-driven deformations give a more realistic appearance.\\
...
Taking the female warrior face shown in Figure \ref{fig:face}\subref{fig:face_sketch} as an example, we make a comparison 3D facial detail generation with polygon modelling and our proposed sketch-based and ODE-driven shape modelling. With Maya polygon modelling, about 40 hours are used to create the detailed facial model in Figure \ref{fig:face}\subref{fig:face_traditional}. With our proposed sketch-based and ODE-driven shape modelling, a curve network shown in Figure \ref{fig:face}(c) which defines the female warrior face is first generated. Then, the curve network is decomposed into different groups of sketches and our developed ODE-driven deformations are used to automatically create surface patches from different groups of sketches. All the created surface patches are naturally and smoothly connected to generate the detailed facial model shown in Figure \ref{fig:face}\subref{fig:face_skm}. The total time of obtaining the detailed facial model in Figure \ref{fig:face}\subref{fig:face_skm} including generating the facial curve network in Figure \ref{fig:face}\subref{fig:face_segments} is 5 hours. Clearly, our proposed sketch-based and ODE-driven shape generator is more efficient in creating detailed facial models than the polygon modelling. Compared to the approach of manipulating primitives in one view plane, our presented approach can create more detailed 3D shapes.
\begin{figure}[!h]
\centering
\begin{subfigure}{0.24\linewidth}
\includegraphics[height=1in]{./figure/facesketch.jpg}\caption{}\label{fig:face_sketch}
\end{subfigure}
\begin{subfigure}{0.24\linewidth}
\includegraphics[height=1in]{./figure/face_traditional.png}\caption{}\label{fig:face_traditional}
\end{subfigure}
\begin{subfigure}{0.24\linewidth}
\includegraphics[height=1in]{./figure/face_segments.png}\caption{}\label{fig:face_segments}
\end{subfigure}
\begin{subfigure}{0.24\linewidth}
\includegraphics[height=1in]{./figure/face_skm.png}\caption{}\label{fig:face_skm}
\end{subfigure}
\caption{Comparison of creating 3D facial details with the polygon modelling and sketch-based and ODE-driven shape generator.}
\label{fig:face}
\end{figure}
%------------------------------------------------------------------------
\section{Conclusions and Future work}\label{conclusions_and_future_work}
In this paper, we have presented a new modelling approach to create detailed 3D models from 2D sketches quickly. By introducing ODE-driven geometric modelling, we have developed a primitive deformer to deform primitives to match the 2D sketches and a shape generator to create new 3D local shapes from use?s drawn strokes, and demonstrated how to quickly create detailed 3D models with physical realism.\\
Compared existing sketch-based modelling, our proposed approach is physics-based and able to create physically realistic and detailed 3D models from 2D sketches easily and quickly. It is more powerful in deforming and manipulating primitives to obtain more detailed shapes. \\
Compared to traditional polygon modelling technology, our proposed approach has the advantages of: 1) easier for beginners to create detailed 3D models, 2) avoiding heavy manual operations, 3) quicker in modelling complicated parts of character models.\\
Although the modelling examples discussed in this paper are a female warrior model and human part models, the developed approach is applicable to other organic models and man-made models. In the future, we will extend its applications to geometric modelling of various organic and man-made models.
