...
\end{subfigure}
\begin{subfigure}{0.32\linewidth}
\includegraphics[height=0.9in]{./figure/head.png}\caption{}\label{fig:head_mesh}
\end{subfigure}
...
Using the same treatment, we can obtain the generator at the other positions. From these obtained generators, a local detail 3D model is created and depicted in (b) of Figure \ref{fig:bum}.\\
\begin{figure}[!htb]
\centering
\begin{subfigure}{0.45\linewidth}
\includegraphics[height=1in]{./figure/referencebum.png}\caption{}\label{reference_bum}
\end{subfigure}
\begin{subfigure}{0.45\linewidth}
\includegraphics[height=1in]{./figure/buttcheck.png}\caption{}\label{buttcheck}
\end{subfigure}
\caption{creation of two open and one closed silhouette contours in three different view planes}
\label{fig:bum}
\end{figure}
...
\begin{equation}
\begin{split}
d_0=d_2-2T_{5,0}\triangle u \\
d_(2N+2)=d_{2N}+2T_{5,1}\triangle u
\end{split}
\label{2endsdelta}
\end{equation}\\
For each of the inner nodes 2, 3, \ldots, $2N-1$, $2N$, we can write a finite difference equation. Since the coordinate values for all the nodes on the curve $c_6$ are known, we can calculate the sculpting force $\mathbf{f_i}$$(i=2,3,\ldots,2N-1,2N)$ from these equations,. We denote these sculpting forces as $\mathbf{f_{5,i}=f_i}$$(i=2,3,\ldots,2N-1,2N)$. With the same method, we can obtain the sculpting forces $\mathbf{f_{5,i}=f_i}$$(i=2,3,\ldots,2N-1,2N)$ acting on the curve $c_6$.\\
...
\centering
 \begin{subfigure}{0.45\linewidth}
 \includegraphics[height=1in]{./figure/palmwithfingers.png}\caption{}\label{fig:palmwithfingers}
 \end{subfigure}
 \begin{subfigure}{0.45\linewidth}
 \includegraphics[height=1in]{./figure/palm.png}\caption{}\label{fig:palmmesh}
  \end{subfigure}
 \caption{palm model creation from two open and one closed silhouette contours in three different view planes}
 \label{fig:palm}
\end{figure}  
With the above method, we draw a closed palm contour and two open curves shown in Figure \ref{fig:palm}\subref{fig:palmwithfingers}. They divide the palm into four regions. One surface is created each of the four regions. Figure \ref{fig:palm}\subref{fig:palmmesh} depicts the palm model consisting of the four surfaces. 
The above method can be also extended to deal with the situations where the two open curves do not intersect. For such situations, the intersecting point of the two open curves becomes the upper closed curve as indicated in Figures \ref{fig:shoulder_and_hip}, and a sweeping surface is encircled by four curves. With this extension, we created 3D shoulder and hip models and depicted them in Figures \ref{fig:shoulder_and_hip}\subref{fig:shoulder} and Figure \ref{fig:shoulder_and_hip}\subref{fig:hip}, respectively. \\
\begin{figure}[!h]
\centering
 \begin{subfigure}{0.45\linewidth}
 \includegraphics[height=0.1\textheight]{./figure/uppertorso.png}\caption{}\label{fig:shoulder}
  \end{subfigure}
\begin{subfigure}{0.45\linewidth}
\includegraphics[height=0.1\textheight]{./figure/hip.png}\caption{}\label{fig:hip}
  \end{subfigure}
 \caption{shoulder and hip model creation from two open and one closed silhouette contours}
 \label{fig:shoulder_and_hip}
\end{figure}  
...
By adding local shapes and smooth transition surfaces, the base mesh shown in Figure \ref{fig:system_overview}\subref{fig:primitives_after_deformation} is changed into that depicted in Figure \ref{fig:system_overview}\subref{fig:blending_body_parts}. \\
%------------------------------------------------------------------------
