...
void CentreAngle(int,double &,double &,double &,double *,double *,double *,double *,double *, double *,
				 double *,double *);
...
void init(void) 
{
    ...
    double xm[50][250],ym[50][250],zm[50][250],x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,
		...
		zcentre[50],anglex[50][200],angley[50][200],anglez[50][200],anglem,a0[70],b0[70],a0u,a0b,b0u,b0b,dpi,
		ai[50][70],bi[50][70];
	char szLine[256];
    Point contour1[228], contour2[228], contour3[228], contour4[228];
...
*/
	...
	for(i=0;i<228;i++)
	{
         fscanf(in1, "%f %f %f", &contour1[i].x, &contour1[i].y, &contour1[i].z);
		 fprintf(out2,"-p   %le %le %le\n",contour1[i].x,contour1[i].y,contour1[i].z);
	}
	fprintf(out2,";\n");
	fprintf(out2,"curve -d 3\n");
	for(i=0;i<228;i++)
	{
         fscanf(in1, "%f %f %f", &contour2[i].x, &contour2[i].y, &contour2[i].z);
		 fprintf(out2,"-p   %le %le %le\n",contour2[i].x,contour2[i].y,contour2[i].z);
	}
	fprintf(out2,";\n");
 	fprintf(out2,"curve -d 3\n");
	for(i=0;i<228;i++)
	{
         fscanf(in1, "%f %f %f", &contour3[i].x, &contour3[i].y, &contour3[i].z);
		 fprintf(out2,"-p   %le %le %le\n",contour3[i].x,contour3[i].y,contour3[i].z);
	}
	fprintf(out2,";\n");
 	fprintf(out2,"curve -d 3\n");
	for(i=0;i<228;i++)
	{
         fscanf(in1, "%f %f %f", &contour4[i].x, &contour4[i].y, &contour4[i].z);
		 fprintf(out2,"-p   %le %le %le\n",contour4[i].x,contour4[i].y,contour4[i].z);
	}
	fprintf(out2,";\n");
	int k,ic,imic;
	im[0]=im[1]=im[2]=im[3]=228;
	ic=0;
	imic=im[ic];
	for(j=0;j<=imic-1;j++)
	{
		xm[ic][j]=contour1[j].x;
		ym[ic][j]=contour1[j].y;
		zm[ic][j]=contour1[j].z;
	}
	ic=1;
	imic=im[ic];
	for(j=0;j<=imic-1;j++)
	{
		xm[ic][j]=contour2[j].x;
		ym[ic][j]=contour2[j].y;
		zm[ic][j]=contour2[j].z;
	}
	ic=2;
	imic=im[ic];
	for(j=0;j<=imic-1;j++)
	{
		xm[ic][j]=contour3[j].x;
		ym[ic][j]=contour3[j].y;
		zm[ic][j]=contour3[j].z;
	}
	ic=3;
	imic=im[ic];
	for(j=0;j<=imic-1;j++)
	{
		xm[ic][j]=contour4[j].x;
		ym[ic][j]=contour4[j].y;
		zm[ic][j]=contour4[j].z;
	}
/*
	for(i=0; i <= icurve-1; i++)
	{
...
*/
	...
	double xcentreic,ycentreic,zcentreic,angleix[250],angleiy[250],angleiz[250],xmic[250],ymic[250],zmic[250],aiic[70],
		biic[70];
//  Calculate the center of the ith curve and determine the angle of each curve point.
	for(ic=0; ic<= icurve-1; ic++)
	{
		...
		CentreAngle(imic,xcentreic,ycentreic,zcentreic,angleix,ngleiy,ngleiz,xmic,ymic,zmic,aiic,biic);
		xcentre[ic]=xcentreic;
		ycentre[ic]=ycentreic;
		zcentre[ic]=zcentreic;
		for(j=0;j<=imic-1;j++)
		{
		anglex[ic][j]=angleix[j];
		angley[ic][j]=angleiy[j];
		anglez[ic][j]=angleiz[j];
		}
	}
/*
//  Order the data according to the angle from 0 to 2*pi.
	int j0;
	double anglemin, anglemax,angleg[50][250],xmg[50][250],ymg[50][250],zmg[50][250];
	for(ic=0; ic<= icurve-1; ic++)
	{
		anglemin=0.0;
		anglemax=1.e10;
...
//  The curves can be well approximated with the constant term a0.Therefore,the following algorithm is very good.
	for(ic=0; ic<=icurve-1;ic++)
	{
		...
		for(j=0;j<=im[ic]-1;j++)
		{
			angleix[j]=anglex[ic][j];
			angleiy[j]=angley[ic][j];
			angleiz[j]=anglez[ic][j];
			xmic[j]=xm[ic][j];
			ymic[j]=ym[ic][j];
			zmic[j]=zm[ic][j];
		}
		printf("ic= %d\n",ic);
		Gellipse(jb,imic,xcentreic,ycentreic,zcentreic,angleix,angleiy,angleiz,xmic,zmic,aiic,biic,ciic);
		for(j=0;j<=im[ic]-1;j++)
		{
			ai[ic][j]=aiic[j];
			bi[ic][j]=biic[j];
...
int Gellipse(int jb1,int imic,double xcentreic,double ycentreic,double zcentreic,
			 double *angleix,double *angleiy,double *angleiz,double *xmic,double *zmic,double *aiic,double *biic,double *ciic)
{
//  fitting of generalized ellipe using x=xc+a0+(j=1 to J)(a2j-1*cos(j*angle)+a2j*sin(j*angle) and z=zc+b0+(j=1 to J)(b2j-1)*sin(j*angle)+b2j*cos(j*angle)
//  The curves can be well approximated with the constant term a0.Therefore,the following algorithm is very good.
	...
	n=2*jb+1;
		...
		for(j=1;j<=jb;j++)
		{
			...
			for(i=0; i <= imic-1; i++)
			{
				aax[0][2*j-1]=aax[0][2*j-1]+cos(j*angleix[i]);
				aax[0][2*j]=aax[0][2*j]+sin(j*angleix[i]);
			}
		}
		for(j=1;j<n;j++)
		{
...
void CentreAngle(int imic,double &xcentreic,double &ycentreic,double &zcentreic,
				 double *angleix,double *angleiy,double *angleiz,double *xmic,double *ymic, double *zmic,double *aiic,
				 double *biic)
{
	int j;
	double anglem;
...
//	fprintf(out2,";\n");
	...
	{
		angleix[j]=0.0;
		anglem=(xmic[j]-xcentreic)/sqrt((xmic[j]-xcentreic)*(xmic[j]-xcentreic)
			+(ymic[j]-ycentreic)*(ymic[j]-ycentreic)+(zmic[j]-zcentreic)*(zmic[j]
			-zcentreic));
		anglem=asin(anglem);
		if((zmic[j]-zcentreic)>=0.0 && (xmic[j]-xcentreic)>=0.0)angleix[j]=anglem; 
		if((zmic[j]-zcentreic)<=0.0 && (xmic[j]-xcentreic)> 0.0)angleix[j]=pi-anglem; 
		if((zmic[j]-zcentreic)< 0.0 && (xmic[j]-xcentreic)< 0.0)angleix[j]=pi-anglem; 
		if((zmic[j]-zcentreic)> 0.0 && (xmic[j]-xcentreic)< 0.0)angleix[j]=2.0*pi+anglem; 
	}
	for(j=0; j <= imic-1; j++)
	{
		angleiy[j]=0.0;
		anglem=(ymic[j]-ycentreic)/sqrt((xmic[j]-xcentreic)*(xmic[j]-xcentreic)
			+(ymic[j]-ycentreic)*(ymic[j]-ycentreic)+(zmic[j]-zcentreic)*(zmic[j]
			-zcentreic));
		anglem=asin(anglem);
		if((ymic[j]-ycentreic)>=0.0 && (xmic[j]-xcentreic)>=0.0)angleiy[j]=anglem; 
		if((ymic[j]-ycentreic)>=0.0 && (xmic[j]-xcentreic)< 0.0)angleiy[j]=pi-anglem; 
		if((ymic[j]-ycentreic)< 0.0 && (xmic[j]-xcentreic)< 0.0)angleiy[j]=pi-anglem; 
		if((ymic[j]-ycentreic)< 0.0 && (xmic[j]-xcentreic)> 0.0)angleiy[j]=2.0*pi+anglem; 
	}
	for(j=0; j <= imic-1; j++)
	{
		angleiz[j]=0.0;
		anglem=(zmic[j]-zcentreic)/sqrt((xmic[j]-xcentreic)*(xmic[j]-xcentreic)
			...
			-zcentreic));
		anglem=asin(anglem);
		if((zmic[j]-zcentreic)>=0.0 && (ymic[j]-ycentreic)>=0.0)angleiz[j]=anglem; 
		if((zmic[j]-zcentreic)>=0.0 && (ymic[j]-ycentreic)< 0.0)angleiz[j]=pi-anglem; 
		if((zmic[j]-zcentreic)< 0.0 && (ymic[j]-ycentreic)< 0.0)angleiz[j]=pi-anglem; 
		if((zmic[j]-zcentreic)< 0.0 && (ymic[j]-ycentreic)> 0.0)angleiz[j]=2.0*pi+anglem; 
	}
}
