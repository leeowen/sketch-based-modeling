...
int dcinv(double **,int);
void damul(double **,double **,int,int,int,double **);
...
//   draw the original curves with random data
/*
	for(i=0; i <= icurve-1; i++)
	{
...
*/
	...
	for(i=icurve-1; i <= icurve-1; i++)
	{
       ...
       for(j=93; j <= 105; j++)
		{
			fprintf(out2,"-p   %le %le %le\n",xmg[i][j],ymg[i][j],zmg[i][j]);
			fprintf(out,"i,j,angleg[i][j]= %d %d %le\n",i,j,angleg[i][j]);
		}
		fprintf(out2,";\n");
	}

	int im1[10];
	double cx11[10][200],cy11[10][200],cz11[10][200],cx21[10][200],cy21[10][200],cz21[10][200];
	i=icurve-1;
	k=0;
	for(j=93; j <= 105; j++)
	{
		...
		cy11[0][k]=ymg[i][j];
		cz11[0][k]=zmg[i][j];
//		fprintf(out,"$$$ j,cx-y-z11[i][j]= %d %le %le %le\n",j,cx11[i][11],cy11[i][11],cz11[i][11]);
		k=k+1;
	}
	im1[0]=k;
	k=0;
	for(j=105; j <= im[i]-1; j++)
	{
		cx11[1][k]=xmg[i][j];
		cy11[1][k]=ymg[i][j];
		cz11[1][k]=zmg[i][j];
...
/*
	for(j=0; j <= im1[0]-1; j++)
	{
		fprintf(out,"j,cx-y-z11[0][j]= %d %le %le %le\n",j,cx11[0][j],cy11[0][j],cz11[0][j]);
	}
	for(j=0; j <= im1[1]-1; j++)
	{
		fprintf(out,"j,cx-y-z11[1][j]= %d %le %le %le\n",j,cx11[1][j],cy11[1][j],cz11[1][j]);
	}
*/
	for(ic=0; ic<=1;ic++)
	{
		imic=im1[ic];
		xcentreic=xcentre[icurve-1];
		ycentreic=ycentre[icurve-1];
		zcentreic=zcentre[icurve-1];
		for(j=0;j<=im1[ic]-1;j++)
		{
			angleic[j]=angle[icurve-1][j];
			xmic[j]=xm[ic][j];
			zmic[j]=zm[ic][j];
		}
	...
	dpi=pi/100.0;
//  draw ellipses with x=xc+a*cos(angle) and z=zc+b*sin(angle) and random data
/*
	for(i=0; i <= icurve-1; i++)
	{
 		fprintf(out2,"curve -d 3\n");
...
*/
//  draw ellipses with x=xc+a*cos(angle) and z=zc+b*sin(angle) and ordered data
/*
	for(i=0; i <= icurve-1; i++)
	{
 		fprintf(out2,"curve -d 3\n");
...
//		fprintf(out2,";\n");
	...
	double *xcentrep = new double [50];
	double *ycentrep = new double [50];
	double *zcentrep = new double [50];
...
//      with aip[ic0+2][] and the boundary curve at ic0+2 represented with aip[ic0+2][].
	...
	id=2;
	ic0=0;
	double **aip = new double* [50];
	double **bip = new double* [50];
	for(i=0; i<50; i++)
    {
		aip[i] = new double [70];
		bip[i] = new double [70];
	...
	double **bba = new double* [nt];
	double **cca = new double* [nt];
	for(i=0; i<nt; i++)
    {
		aa[i] = new double [nt];
		bba[i] = new double [nt];
	...
	du=1.0/(icurve-1);
	for(k=0;k<=icurve-1;k++)
	{
		uk=k*du;
		for(m=0;m<=icurve-1;m++)
		{
            if(uk==0.0)
			{
				if(m==0)aa[k][m]=1.0;
	...
	for(i=0;i<=icurve-1;i++)
	for(j=0;j<=icurve-1;j++)bba[i][j]=aa[i][j];
	dcinv(aa,icurve);
	damul(bba,aa,icurve,icurve,icurve,cca);

	double sx[icurve],sy[icurve],sz[icurve],fx[icurve],fy[icurve],fz[icurve],u0,v0;
	ns=50;
	du=1.0/ns;
	dv=2.0*pi/ns;
	for(i=0;i<=ns;i++)
	{
		...
		for(k=0;k<=ns;k++)
		{
			...
			for(m=0;m<=icurve-1;m++)
			{
                sx[m]=xcentre[m]+ai[m][0];				
				sy[m]=ycentre[m];
				sz[m]=zcentre[m]+bi[m][0];
				for(j=1;j<=jb;j++)
				{
					sx[m]=sx[m]+ai[m][2*j-1]*cos(j*v0)+ai[m][2*j]*sin(j*v0);
					sy[m]=sy[m]+0.0;
					sz[m]=sz[m]+bi[m][2*j-1]*sin(j*v0)+bi[m][2*j]*cos(j*v0);
//				if(i==0 && k==0 && m==0)printf("j,ai[m][2*j-1],ai[m][2*j],bi[m][2*j-1],bi[m][2*j]= %d %le %le %le %le\n",
//					j,ai[m][2*j-1],ai[m][2*j],bi[m][2*j-1],bi[m][2*j]);
				}
//				if(i==0 && k==0)printf("m,sx[m],sz[m]= %d %le %le\n",m,sx[m],sz[m]);
			}
			...
			{	
				...
				for(m=0;m<=icurve-1;m++)
				{
					fx[j]=fx[j]+aa[j][m]*sx[m];
					fy[j]=fy[j]+aa[j][m]*sy[m];
					fz[j]=fz[j]+aa[j][m]*sz[m];
				}
//				if(i==0 && k==0)printf("j,fx[j],fz[j]= %d %le %le\n",j,fx[j],fz[j]);
			}
			for(m=0;m<=icurve-1;m++)
			{
				u0if(m,u0,ukm);
...
/*
	for(i=0;i<=icurve-1;i++)
	for(j=0;j<=icurve-1;j++)printf("i,j,cca[i][j]= %d %d %le\n",i,j,cca[i][j]);
*/

/*
	for(k=0;k<=icurve-1;k++)
		for(m=0;m<=icurve-1;m++)
			printf("k,m,aa[k][m]= %d %d %le\n",k,m,aa[k][m]);
*/
