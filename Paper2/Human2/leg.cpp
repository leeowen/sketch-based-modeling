...
void init(void) 
{
    int i,j,n,n0,l,m,im[100],iu,iv,jj,iuu,icurve; 
...
/*
	...
	for(i=0; i <= icurve-1; i++)
	{
       for(j=0; j <= im[i]-2; j++)
		{
			glBegin(GL_LINES);
				glVertex3f (xm[i][j],ym[i][j],ym[i][j]);
				glVertex3f (xm[i][j+1],ym[i][j+1],ym[i][j+1]);
			glEnd(); 
		}
	}
*/
...
//  The curves can be well approximated with the constant term a0.Therefore,the following algorithm is very good.
	for(ic=0; ic<=icurve-1;ic++)
	{
		...
		for(j=0;j<=2*jb;j++)
		{
			ai[ic][j]=aiic[j];
			bi[ic][j]=biic[j];
			fprintf(out,"$$$$  ic,j,ai[ic][j],bi[ic][j]= %d %d %le %le\n",ic,j,ai[ic][j],bi[ic][j]);
		}
	}
//  draw generalized ellipses with x=xc+a0+(j=1 to J)(a2j-1*cos(j*angle)+a2j*sin(j*angle) and 
//	z=zc+b0+(j=1 to J)(b2j-1)*sin(j*angle)+b2j*cos(j*angle)
    int ns=50;
	double anglei;
	du=1.0/ns;
	dv=2.0*pi/ns;
