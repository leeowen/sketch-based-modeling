...
void init(void) 
{
    ...
    double xm[50][200],ym[50][200],zm[50][200],x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,
		...
		nz3,nz4,nxyz,txi[100],tyi[100],tzi[100],tx1,ty1,tz1,xcentre[100],ycentre[100],
		zcentre[100],angle[70][200],anglem,a0[70],b0[70],a0u,a0b,b0u,b0b,dpi,
		ai[70][200],bi[70][200];
	char szLine[256];
...
/*
	...
	iu=7;
	iv=6;
	dx=(xm[0][iu]-xm[0][iv])/m;
	dy=(ym[0][iu]-ym[0][iv])/m;
	dz=(zm[0][iu]-zm[0][iv])/m;
	for(i=1;i<=m-1;i++)
	{
		xm[0][im[0]-1+i]=xm[0][iv]+i*dx;
		ym[0][im[0]-1+i]=ym[0][iv]+i*dy;
		zm[0][im[0]-1+i]=zm[0][iv]+i*dz;
	}
	im[0]=im[0]+m-1;
	fprintf(out,"im[1]= %d\n",im[0]);
	for(i=1;i<=im[0]-1;i++)
	{
		fprintf(out,"i,xm[0][i],ym[0][i],zm[0][i]= %d %le %le %le\n",i,xm[0][i],ym[0][i],zm[0][i]);
	}
*/
...
//		fprintf(out2,";\n");
	...
	for(i=0; i <= icurve-1; i++)
	{
        for(j=0; j <= im[i]-1; j++)
		{
			...
			anglem=asin(anglem);
			if((zm[i][j]-zcentre[i])>=0.0 && (ym[i][j]-ycentre[i])>=0.0)angle[i][j]=anglem; 
			if((zm[i][j]-zcentre[i])>=0.0 && (ym[i][j]-ycentre[i])< 0.0)angle[i][j]=pi-anglem; 
			if((zm[i][j]-zcentre[i])< 0.0 && (ym[i][j]-ycentre[i])< 0.0)angle[i][j]=pi-anglem; 
			if((zm[i][j]-zcentre[i])< 0.0 && (ym[i][j]-ycentre[i])> 0.0)angle[i][j]=2.0*pi+anglem; 
		}
	}
...
//		    fprintf(out2,"-p   %le %le %le\n",xmg[i][0],ymg[i][0],zmg[i][0]);
	...
	for(i=0; i <= icurve-1; i++)
	{
        ...
        for(j=0; j <= im[i]-1; j++)
		{
            a0u=a0u+(ym[i][j]-ycentre[i])*cos(angle[i][j]);
...
/*
	for(i=0; i <= icurve-1; i++)
	{
		...
		{
            x1=xcentre[i];
		    y1=ycentre[i]+a0[i]*cos(angleg[i][j]);
...
/*	for(ic=0; ic <= icurve-1; ic++)
	{
		...
		for(i=0; i <= im[ic]-1; i++)
		{
			bbx[0]=bbx[0]+(ym[ic][i]-ycentre[ic]);
			aax[0][0]=aax[0][0]+1.0;
		}
		for(j=1;j<=jb;j++)
		{
			...
			for(i=0; i <= im[ic]-1; i++)
			{
				aax[0][2*j-1]=aax[0][2*j-1]+cos(j*angle[ic][i]);
				aax[0][2*j]=aax[0][2*j]+sin(j*angle[ic][i]);
			}
		}
		for(j=1;j<n;j++)
		{
		...
		for(k=1;k<=jb;k++)
		{
			for(i=0; i <= im[ic]-1; i++)
			{
				bbx[k]=bbx[k]+(ym[ic][i]-ycentre[ic])*cos(k*angle[ic][i]);
				bbx[jb+k]=bbx[jb+k]+(ym[ic][i]-ycentre[ic])*sin(k*angle[ic][i]);
				aax[k][0]=aax[k][0]+cos(k*angle[ic][i]);
				aax[jb+k][0]=aax[jb+k][0]+sin(k*angle[ic][i]);
			}
			for(j=1;j<=jb;j++)
			{
				for(i=0; i <= im[ic]-1; i++)
				{				
					aax[k][2*j-1]=aax[k][2*j-1]+cos(j*angle[ic][i])*
						cos(k*angle[ic][i]);			
					aax[k][2*j]=aax[k][2*j]+sin(j*angle[ic][i])*
						cos(k*angle[ic][i]);
					aax[jb+k][2*j-1]=aax[jb+k][2*j-1]+cos(j*angle[ic][i])*
						sin(k*angle[ic][i]);			
					aax[jb+k][2*j]=aax[jb+k][2*j]+sin(j*angle[ic][i])*
						sin(k*angle[ic][i]);
				}
			}
		}
	...
	for(ic=0; ic <= icurve-1; ic++)
	{
		...
		for(j=1;j<=jb;j++)
		{
			...
			for(i=0; i <= im[ic]-1; i++)
			{
				aax[0][2*j-1]=aax[0][2*j-1]+sin(j*angle[ic][i]);
				aax[0][2*j]=aax[0][2*j]+cos(j*angle[ic][i]);
			}
		}
		for(j=1;j<n;j++)
		{
		...
		for(k=1;k<=jb;k++)
		{
			for(i=0; i <= im[ic]-1; i++)
			{
				bbx[k]=bbx[k]+(zm[ic][i]-zcentre[ic])*sin(k*angle[ic][i]);
				bbx[jb+k]=bbx[jb+k]+(zm[ic][i]-zcentre[ic])*cos(k*angle[ic][i]);
				aax[k][0]=aax[k][0]+sin(k*angle[ic][i]);
				aax[jb+k][0]=aax[jb+k][0]+cos(k*angle[ic][i]);
			}
			for(j=1;j<=jb;j++)
			{
				for(i=0; i <= im[ic]-1; i++)
				{				
					aax[k][2*j-1]=aax[k][2*j-1]+sin(j*angle[ic][i])*
						sin(k*angle[ic][i]);			
					aax[k][2*j]=aax[k][2*j]+cos(j*angle[ic][i])*
						sin(k*angle[ic][i]);
					aax[jb+k][2*j-1]=aax[jb+k][2*j-1]+sin(j*angle[ic][i])*
						cos(k*angle[ic][i]);			
					aax[jb+k][2*j]=aax[jb+k][2*j]+cos(j*angle[ic][i])*
						cos(k*angle[ic][i]);
				}
			}
		}
...
*/
	...
	for(ic=0; ic <= 0; ic++)
	{
		...
		for(i=0; i <= im[ic]-1; i++)
		{
			bbx0[i]=ym[ic][i]-ycentre[ic];
			aax0[i][0]=1.0;
			for(j=1;j<=jj;j++)
			{
				aax0[i][2*j-1]=cos(j*angle[ic][i]);
				aax0[i][2*j]=sin(j*angle[ic][i]);
			}
		}
	    ciggj(aax0,im[ic],bbx0);
		for( k=0;k<=im[ic]-1;k++)ai[ic][k]=bbx[k];
		...
		for(i=0; i <= im[ic]-1; i++)
		{
			bbx0[i]=zm[ic][i]-zcentre[ic];
			aax0[i][0]=1.0;
			for(j=1;j<=jj;j++)
			{
				aax0[i][2*j-1]=sin(j*angle[ic][i]);
				aax0[i][2*j]=cos(j*angle[ic][i]);
			}
		}
	    ciggj(aax0,im[ic],bbx0);
		for( k=0;k<=im[ic]-1;k++)bi[ic][k]=bbx[k];
...
//  draw generalized ellipses with x=xc+a0+(j=1 to J)(a2j-1*cos(j*angle)+a2j*sin(j*angle) and z=zc+b0+(j=1 to J)(b2j-1)*sin(j*angle)+b2j*cos(j*angle)
	...
	for(ic=0; ic <= icurve-1; ic++)
	{
		...
		for(i=0; i <= ns-1; i++)
		{
			anglei=i*dv;
			x1=xcentre[ic];
			y1=ycentre[ic]+ai[ic][0];
			z1=zcentre[ic]+bi[ic][0];
			for(int j=1;j<=jj;j++)
			{        

                x1=x1+0.0;
				y1=y1+ai[ic][2*j-1]*cos(j*anglei)
...
*/
exit(1);
	double u1,u2,u22,u23,u3,u32,u33,D,D10,D11,D12,D13,D20,D21,D22,D23,D30,D31,D32,D33,s3v,u0,v0;
	u1=0.455;
	u12=u1*u1;
	u13=u1*u12;
	u2=0.77;
	...
	D22=u1*u33-u3*u13;
	D23=u13*u2-u23*u1;
	D30=u1*u22+u12*u3+u2*u32-u3*u22-u1*u32-u12*u2;
	D31=u2*u32-u3*u22;
	D32=u12*u3-u1*u32;
	D33=u1*u22-u12*u2;
	for(i=0;i<=ns;i++)
	{
		...
		{
			v0=k*dv;
			s3v=xcentre[icurve-1]+ai[icurve-1][0];
		}
	}
