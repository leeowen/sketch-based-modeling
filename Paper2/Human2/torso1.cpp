...
//			fprintf(out,"ic,k,angleg[ic][k]= %d %d %le\n",ic,k,angleg[ic][k]);
			xmg[ic][k]=xm[ic][j0];
			ymg[ic][k]=ym[ic][j0];
			zmg[ic][k]=zm[ic][j0];
			anglemin=anglemax;
...
/*
	for(i=0; i <= icurve-1; i++)
	{
       ...
       for(j=0; j <= im[i]-1; j++)
		...
		fprintf(out2,"`;\n");
		fprintf(out2,"fitBspline -ch 1 -tol 0.01 $curveName;\n");
		fprintf(out2,"delete $curveName;\n");
	}
*/
	for(i=0; i <= icurve-1; i++)
	{
...
/*
	int iang0,iang1,iang2,iang34;
	double dangle[icurve][200];
	for(i=0; i <= icurve-1; i++)
	{
       for(j=0; j <= im[i]-2; j++)
       {
		   if(j==0)dangle[i][j]=angleg[i][j]-0.0;
           if(j!=0)dangle[i][j]=angleg[i][j+1]-angleg[i][j];
//		   printf("i,j,dangle[i][j]= %d %d %le\n",i,j,dangle[i][j]);
		}
	}
	...
	for(i=29; i <= 34; i++)
	{
		...
		ip[i][2]=iang1;
		ip[i][3]=im[i]-1;
//	   printf("i,iang0= %d %d %le %le %le\n",i,iang0,xmg[i][iang0],ymg[i][iang0],zmg[i][iang0]);
//	   printf("i,iang1= %d %d %le %le %le\n",i,iang1,xmg[i][iang1],ymg[i][iang1],zmg[i][iang1]);
		printf("%d %d %d %d %d\n",i,ip[i][0],ip[i][1],ip[i][2],ip[i][3]);
	}


//				if((i== 31) && ((k-i) <= 5) && fabs(xmg[i][k] - xmg[i][j])<0.5 && fabs(zmg[i][k] + zmg[i][j])<0.5)
//					printf("i,j,k,xmg[i][j],xmg[i][k]= %d %d %d %le %le %le %le\n",i,j,k,xmg[i][j],xmg[i][k],zmg[i][j],zmg[i][k]);


//	icsize=4*(icurve1-icurve0+1);
	double v0,vj,cca[4][4],ccb[4],ccab[4][4],cp[3][4],xc[4],xmgb[300],ymgb[300],zmgb[300],wt=5.0;
	...
	nv=9;
		...
		for(i=ip2;i<=ip3;i++)
		{
            xmgb[ip1-ip0+nv+i-ip2]=xmg[curvei][i];
            ymgb[ip1-ip0+nv+i-ip2]=ymg[curvei][i];
            zmgb[ip1-ip0+nv+i-ip2]=zmg[curvei][i];
		}
...
//			printf("i,xmg[curvei][ip0+i]= %d %le\n",i,xmg[curvei][ip3-i]);
		}

//		for(i=0;i<=im[curvei]-1;i++)fprintf(out,"i,xmg[curvei][i]= %d %le\n",i,xmg[curvei][i]);
...
//			printf("k,xmgb[k],ymgb[k],zmgb[k]= %d %le %le %le\n",k,xmgb[k],ymgb[k],zmgb[k]);
		}

 		k=ip1-ip0+nv+ip3-ip2+nv-1;
		fprintf(out2,"curve -d 3\n");
        for(j=0; j <= k; j++)
		{
