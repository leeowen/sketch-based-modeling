...
class Canvas(QtWidgets.QDialog):
    ...
    def draw_generalisedEllipse(self,painter):
        ...
        if self.single_piece_mode==True:
            ...
            if self.manualJ_mode==True:
                J=self.manualJ
                a_first_half,b_first_half,a_second_half,b_second_half=self.coefficients_solver_half_of_segmented_ellipse(J)
    ...
    def form_vertices_of_segmented_ellipse(self,a1,b1,a2,b2):
        ...
        for i in range(I):
            ...
            di1=math.sqrt(pow(self.vertices_first_half[i][0]-first_half_vertex[0],2)+pow(self.vertices_first_half[i][2]-first_half_vertex[1],2))
            d[i]=di1
            Ea+=(di1/self.d_bar[i])
    ...
    def coefficients_solver_for_half_of_segmented_ellipse(self,J):
        ...
        b1ConstArray=np.zeros(2*J+1)
        b1CoefficientMatrix=np.ndarray(shape=(2*J+1,I), dtype=float, order='C')
        
        a2ConstArray=np.zeros(2*J+1)
        a2CoefficientMatrix=np.ndarray(shape=(2*J+1,I), dtype=float, order='C')# row-major
        
        b2ConstArray=np.zeros(2*J+1)
        b2CoefficientMatrix=np.ndarray(shape=(2*J+1,I), dtype=float, order='C')
        
        for i in range(I):
            a1CoefficientMatrix[0,i]=1.
            b1CoefficientMatrix[0,i]=1.
            a2CoefficientMatrix[0,i]=1.
            b2CoefficientMatrix[0,i]=1.
            
        for i in range(I):# for aCoefficientMatrix's column
            for j in range(1,J+1):# for aCoefficientMatrix's row
                try:
                    vi1=self.angles_first_half[i]
                    vi2=self.angles_second_half[i]
                except IndexError:
                    error_dialog = QtWidgets.QErrorMessage(self)
                    error_dialog.showMessage('vertices data is empty, please choose a data file first')
                else:    
                    a1CoefficientMatrix[2*j-1,i]=math.cos(vi1*j)
                    a1CoefficientMatrix[2*j,i]=math.sin(vi1*j)
                    a2CoefficientMatrix[2*j-1,i]=math.cos(vi2*j)
                    a2CoefficientMatrix[2*j,i]=math.sin(vi2*j)

                    # aConstAtrray[0] and bConstAtrray[0] always equal to 0 by definition!
                    a1ConstArray[2*j-1]+=(self.vertices_first_half[i][0]-self.center_first_half.x())*math.cos(vi1*j)
                    a1ConstArray[2*j]+=(self.vertices_first_half[i][0]-self.center_first_half.x())*math.sin(vi1*j)
                
                    b1CoefficientMatrix[2*j-1,i]=math.sin(vi1*j)
                    b1CoefficientMatrix[2*j,i]=math.cos(vi1*j)
                   
                    b1ConstArray[2*j-1]+=(self.vertices_first_half[i][2]-self.center_first_half.y())*math.sin(vi1*j)
                    b1ConstArray[2*j]+=(self.vertices_first_half[i][2]-self.center_first_half.y())*math.cos(vi1*j)
                    
                    a2ConstArray[2*j-1]+=(self.vertices_second_half[i][0]-self.center_second_half.x())*math.cos(vi2*j)
                    a2ConstArray[2*j]+=(self.vertices_second_half[i][0]-self.center_second_half.x())*math.sin(vi2*j)
                
                    b2CoefficientMatrix[2*j-1,i]=math.sin(vi2*j)
                    b2CoefficientMatrix[2*j,i]=math.cos(vi2*j)
                   
                    b2ConstArray[2*j-1]+=(self.vertices_second_half[i][2]-self.center_second_half.y())*math.sin(vi2*j)
                    b2ConstArray[2*j]+=(self.vertices_second_half[i][2]-self.center_second_half.y())*math.cos(vi2*j)
                                                
        A1=np.dot(a1CoefficientMatrix,a1CoefficientMatrix.transpose())
        a1=np.linalg.solve(A1,a1ConstArray)   
        B1=np.dot(b1CoefficientMatrix,b1CoefficientMatrix.transpose())      
        b1=np.linalg.solve(B1,b1ConstArray) 
                
        A2=np.dot(a2CoefficientMatrix,a2CoefficientMatrix.transpose())
        a2=np.linalg.solve(A2,a2ConstArray)   
        B2=np.dot(b2CoefficientMatrix,b2CoefficientMatrix.transpose())      
        b2=np.linalg.solve(B2,b2ConstArray)
