...
def calculate_Ea_Em_3D(before_vertices, after_vertices, d_bar, start_index):
    ...
    for i in range(N):
        ...
        if len(before_vertices) == N:
            ...
            d_i = math.sqrt((before_vertices[index][0] - after_vertices[i][0]) ** 2 + (
                        before_vertices[index][1] - after_vertices[i][1]) ** 2 + (before_vertices[index][2] - after_vertices[i][2]) ** 2)
        Ea += (d_i / d_bar[index])
        if Em < d_i / d_bar[index]:
            Em = d_i / d_bar[index]
    Ea = Ea / N
...
def findJ_3D(vertices, angles, d_bar, center, Ea_criteria, Em_criteria, start_index):
    ...
    for axis in [0, 1, 2]:# x,y,z axis
        Ea = 99999.9
        Em = 99999.9
        while Ea >= Ea_criteria/1.4 or Em >= Em_criteria/1.4:
            J[axis] += 1
...
def findJ_for_non_end_composite_3D(vertices_one_segment, angles, d_bar, segment_center, Ea_criteria, Em_criteria, previous, func_getCoefficients_for_non_end_composite_single_3D):
    ...
    for axis in [0, 1, 2]:  # x,y,z axis
        ...
        for iterate in range(10):#while Ea >= (Ea_criteria/1.5) or Em >= (Em_criteria/1.5):
            ...
            if Ea <= (Ea_criteria/1.4) and Em <= (Em_criteria/1.4):
                break
            else:
                J[axis] += 1
        coe.append(tmp)
